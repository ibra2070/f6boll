...
// --- API routes ---------------------------------------------------------
// Browser routes are handled directly by Vercel rewrites.
// Do not prefix with /api because /auth/* and /record/* are routed to api/index.js.
const API_PREFIX = "";
...
if (!cameraId) {
  ...
  showMsg(
    "This link can’t start a recording because the camera is missing. Please scan the QR code again or ask the staff for help.",
    "err"
  );
}
...
async function checkAvailability(showBusyMessage = true) {
  ...
  try {
    ...
    if (status.available === false) {
      ...
      if (showBusyMessage) {
        showMsg(reservationMessage(status.until), "err");
      }
...
startBtn.onclick = async () => {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      showMsg(
        "Recording started. The system will stop automatically at the scheduled time. Thank you for choosing Hatrick LTD. Your video will be sent after the game.",
        "ok"
      );
